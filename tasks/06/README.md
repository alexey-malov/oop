...
<table>
    <tr>
        ...
        <th>Сообщение для вывода</th>
    </tr>
    <tr>
        <td>Формат URL некорректный, например: отсутствует протокол, отсутствует домен, ееправильный формат порта(порт указан, но не является числом), неправильная структура URL и т.д.</td>
        <td>Invalid URL format</td>
    </tr>
    <tr>
        <td>Протокол отличен от HTTP или HTTPS</td>
        <td>Invalid protocol</td>
    </tr>
    <tr>
        <td>Порт меньше 1 или больше 65535</td>
        <td>Port out of range</td>
    </tr>
    <tr>
        <td>Домен является пустой строкой</td>
        <td>Domain cannot be empty</td>
    </tr>
</table>
...
Если порта нет, нужно пропустить эту строчку в выводе. Если документ не указан, написать корневой путь "/"
