...
Напишите программу, реализующую класс `CStringList`. Программа работает с одним списком строк и поддерживает следующие команды:
...
<table>
    ...
    <tr>
        <td>Добавляет в конец списка строку</td>
        <td>push_back <строка></td>
        <td>ничего не выводит</td>
    </tr>
    <tr>
        <td>Вставляет строку в указанную позицию</td>
        <td>insert <позиция> <строка></td>
        <td>ничего не выводит</td>
    </tr>
    <tr>
        <td>Удаляет элемет в указанной позиции</td>
        <td>erase <позиция></td>
        <td>ничего не выводит</td>
    </tr>
    <tr>
        <td>Удаляет все элементы из списка</td>
        <td>clear</td>
        <td>ничего не выводит</td>
    </tr>
    <tr>
        <td>Выводит количество элементов в списке</td>
        <td>size</td>
        <td><число></td>
    </tr>
    <tr>
        <td>Проверяет списк на пустоту. Выводит TRUE, если пуст, иначе FALSE.</td>
        <td>empty</td>
        <td>TRUE или FALSE</td>
    </tr>
    <tr>
        <td>Выводит все элементы списка через пробел</td>
        <td>print</td>
        <td><элементы списка>, где <элементы списка> ::= <элементы списка> <последний элемент>  | <последний элемент></td>
    </tr>
    <tr>
        <td>Выводит все элементы списка в обратном порядке через пробел</td>
        <td>print_reverse</td>
        <td><элементы списка>, где <элементы списка> ::= <последний элемент> <элементы списка> | <последний элемент></td>
    </tr>
</table>
...
<table>
    <tr>
        ...
        <th>Выводимое сообщение</th>
    </tr>
    <tr>
        <td>Попытка вставки элемента по некорректной позиции (позиция < 0 или позиция > размера списка)</td>
        <td>"Invalid position for insertion"</td>
    </tr>
    <tr>
        <td>Попытка удаления элемента по некорректной позиции (позиция < 0 или позиция >= размера списка)</td>
        <td>"Invalid position for deletion"</td>
    </tr>
    <tr>
        <td>Попытка удаления элемента из пустого списка</td>
        <td>"Cannot delete from empty list"</td>
    </tr>
    <tr>
        <td>Некорректная команда</td>
        <td>"Unknown command"</td>
    </tr>
    <tr>
        <td>Недостаточное количество аргументов для команды</td>
        <td>"Insufficient arguments"</td>
    </tr>
    <tr>
        <td>Слишком много аргументов для команды</td>
        <td>"Too many arguments"</td>
    </tr>
    <tr>
        <td>Некорректный формат числа для позиции (не является целым числом)</td>
        <td>"Invalid position format"</td>
    </tr>
</table>

При завершении работы программы с ошибкой, программа должна завершиться с кодом возврата 0. Это необходимо для корректного тестирования программы в системе Яндекс.Contest.
