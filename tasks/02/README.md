...
Объяснение: считываем 3.659512, как 3.66. Среднее арифметическое - 2.22, прибавляем его к всем элементам.
...
Объяснение: все числа отрицательные, выводим их без изменений, с точностью в три знака после запятой.
...
- При запуске без параметров командной строки программа построчно текст из стандартного потока.
- При запуске из командной строки в качестве параметра передается строка.
