    ...
    - [Задание 6](#задание-6)
      - [Вариант №1 – Expand Template – 100 баллов ](#вариант-1--expand-template--100-баллов-)
        ...
        - [Подробные примеры](#подробные-примеры)
          ...
          - [Пример 2: Подстановка с вложенными параметрами](#пример-2-подстановка-с-вложенными-параметрами)
          - [Пример 3: Выбор наиболее длинного совпадения](#пример-3-выбор-наиболее-длинного-совпадения)
          - [Пример 4: Ошибка дублирования ключа](#пример-4-ошибка-дублирования-ключа)
...
Разработайте приложение expand_template.exe, выполняющее замену шаблонных параметров в тексте на соответствующие значения согласно предоставленным правилам подстановки.
...
- При запуске из командной строки с параметрами программа считывает шаблон из указанного входного файла, использует пары ключ-значение из аргументов и записывает результат в выходной файл.
- Вывод справки при указании параметра `-h`.

##### Правила подстановки

1. Пустые строки в качестве ключа игнорируются
2. Параметр может встречаться в шаблоне многократно
3. При наличии нескольких вариантов подстановки выбирается параметр наибольшей длины
4. Части строки, где уже была выполнена подстановка, повторно не обрабатываются

##### Формат входных данных

При работе через stdin:
1. Первая строка содержит строку-шаблон
2. Последующие строки содержат пары параметр-значение в формате:
```
<key> := <value>
```
где:
...
```
где:
- **`<input-file>`** - путь к файлу, содержащему строку-шаблон
...
Программа должна записать в выходной поток или файл результат замены всех шаблонных параметров на соответствующие значения согласно правилам подстановки. Части текста, не содержащие шаблонных параметров, должны остаться без изменений.

##### Сообщения об ошибках

В зависимости от ошибки в стандартный поток вывода выводятся следующие сообщения:

- Если шаблон пуст: "Empty template"
- Если неверный формат пары параметр-значение  
или отсутствует значение после разделителя: "Invalid input"
- Если ключ определен повторно: "Duplicate key"

##### Требования к тестированию

Для тестирования функционала необходимо разработать набор тестов с использованием одного из популярных C++ фреймворков для тестирования. Тесты должны проверять корректность работы программы на разумном наборе входных параметров.

##### Примеры использования программы

Работа через командную строку:
```sh
expand_template.exe input.txt output.txt "%USER_NAME%" "Ivan Petrov" "{WEEK_DAY}" "Friday"
```
...
```
Объяснение: Каждый параметр в шаблоне (%USER_NAME% и {WEEK_DAY}) заменяется на соответствующее значение. Остальной текст остаётся без изменений.

###### Пример 2: Подстановка с вложенными параметрами
Ввод:
```
Hello, %USER_NAME%. Today is {WEEK_DAY}.
...
```
Объяснение: Параметры в значениях подстановки не обрабатываются повторно, поэтому %USER_NAME% и {WEEK_DAY} внутри значений остаются без изменений.

###### Пример 3: Выбор наиболее длинного совпадения
Ввод:
```
-AABBCCCCCABC+
A := [a]
...
###### Пример 4: Ошибка дублирования ключа
Ввод:
```
Hello, %USER_NAME%.
%USER_NAME% := Ivan Petrov
...
Duplicate key
```
Объяснение: Ключ %USER_NAME% определён дважды с разными значениями, что является ошибкой.
