...
**Не используйте циклы для определения дня недели и перевода из timestamp и обратно — вычислительная сложность операций над датой должна иметь быть O(1). Решение с циклами будет приниматься с коэффициентом 0.6.**
