...
Для команд `EngineOn`, `EngineOff`, `SetGear` и `SetSpeed` какие-либо должны выводиться только в случае возникновения ошибки.

Программа должна быть способна работать в двух режимах:

- При запуске без параметров командной строки программа считывает команды из `stdin` и выводит результаты в `stdout`
  
- При запуске из командной строки с параметром  `-h` программа выводит справку.
...
- Неправильный формат аргумента команды (например, передан нецелочисленный аргумент для команд `SetGear` или `SetSpeed`):
...
- Попытка переключить передачу при выключенном двигателе: "Engine off: cannot set gear"
- Передача, переданная в качестве аргумента, не существует (не находится в диапазоне -1..5): "Invalid gear"
- Попытка переключиться на задний ход (-1) при движении (скорость не равна 0): "Cannot reverse while moving"
- Попытка установить передачу, когда текущая скорость не находится в пределах диапазона для этой передачи:
  "Unsuitable current speed"
- Попытка установить отрицательное значение скорости: "Speed cannot be negative"
- Попытка установить скорость при выключенном двигателе: "Engine off: cannot set speed"
- Попытка увеличить скорость на нейтральной передаче (0): "Cannot accelerate on neutral"
- Попытка установить скорость, выходящую за допустимый диапазон для текущей передачи: "Speed out of gear range"
...
Включаем двигатель, включаем первую передачу, разгоняемся до 19. Пробуем переключиться на вторую передачу,
но получаем сообщение об ошибке: текущая скорость не подходит для второй передачи, т.к. не находится в её диапазоне (20 – 50).
