...
- `SetGear` передача (указывается через пробел). Включает заданную передачу, заданную числом от -1 до 5.
- `SetSpeed` скорость (указывается через пробел). Устанавливает указанную скорость движения, заданную неотрицательным целым числом.
...
- Попытка переключиться на передачу (кроме нейтральной) при выключенном двигателе: "Cannot set gear while engine is off"
...
- Попытка переключиться с задней передачи на переднюю, когда скорость не равна 0:
  "Cannot shift from reverse to forward while moving"
