...
- Неправильный формат аргумента для команд `SetGear` или `SetSpeed`
  (**перекрывает по приоритету все остальные сообщения, связанные с этими командами**):
  "Invalid command argument"

###### Сообщения, связанные с установкой передачи (SetGear)

- Передача из аргумента не находится в диапазоне -1..5
  (**перекрывает по приоритету все следующие сообщения**): "Invalid gear"
...
- Попытка установить отрицательное значение скорости (**перекрывает по приоритету все следующие сообщения**):
