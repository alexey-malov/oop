...
- **Ввод через stdin:** Если программа запускается без параметров,
  она должна принимать первой строкой искомую строку, второй строкой - строка-заменитель,
  затем текст для замены (stdin).
...
Объяснение: программа выведет начальную строку, так как первый параметр (искомая строка) - пустая строка.
...
Объяснение: при умножении любой матрицы на нулевую матрицу (у которой все коэффициенты - нули)
