...
Формат ввода: на вход поступают данные файла включая заголовок в виде 16-ричных чисел парами подряд без разделителей.

**Примечание**: BMP файлы используют little-endian как способ записи многобайтовых чисел.

Пример: число `0x4D42` записывается как `424D`

Формат вывода:
```sh
Resolution: <число> x <число>
Bits per pixel: <число>
Colors in palette: <число>
Image size: <число>
Compression type: <compression_info>
```

*Colors in palette* следует выводить только в случае, если в BMP файле используется палитра (8 и менее бит). Иначе не выводить эту строку.

```sh
<compression_info> ::= ("No compression" | "RLE 8-bit" | "RLE 4-bit" | "Bit fields" | "JPEG" | "PNG")
```
