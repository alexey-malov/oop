...
Формат ввода: на вход первой сторокой поступает команда шифрования или дешифрования(crypt/decrypt), второй сторокой вводится ключ <key>, третьей -  данные в 16-ричной системе счисления. Вводятся парами, подряд, без разделителей.

Если было введено нечетное количество чисел, программа должна завершиться с ошибкой.
```sh
crypt
12
ABCDEF
...
Формат вывода: при успешном выполнении выводится результат шифрования/дешифрования - числа в строку без разделителей в 16-ричной системе счисления.
```sh
123456
```
