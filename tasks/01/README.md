...
Формат ввода: на вход первой сторокой поступает команда компрессии или декомпрессии(pack/unpack), второй сторокой вводится текст.
```sh
pack
AAABBC
```

Формат вывода: при успешном выполнении выводится результат обработки текста - числа в строку без разделителей, при ошибке ERROR.
```sh
034102420143
```
