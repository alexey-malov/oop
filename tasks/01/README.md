...
Разработайте программу **replace.exe**, выполняющую замену всех вхождений подстроки в текстовом файле на другую строку, и записывающей результат в выходной файл (отличный от входного). 
...
- **Ввод через stdin:** Если программа запускается без параметров,
  она должна принимать первой строкой входную строку, второй строкой - подстроку, которую нужно найти, третьей строкой - строку, на которую нужно заменить найденные подстроки (stdin)
...
Объяснение: "1231234" заменяется на "CBO".
...
Объяснение: программа выведет начальную строку, так как второй параметр - пустая строка.
