...
Разработайте приложение **rotatebyte.exe**, выполняющее **циклическое вращение** битов в байте влево или вправо
на заданное количество разрядов. Для решения задачи используйте операции манипулирования битами.
...
Программа должна поддерживать следующие режимы работы:

- При запуске без параметров командной строки программа считывает байт
  и количество битов из stdin и выводит результат в stdout.
- При запуске из командной строки байт и количество битов передаются в качестве параметров,
  а также указывается направление вращения (L для влево и R для вправо).
- Вывод краткой инструкции об использовании. Если в командной строке программы передан параметр `-h`,
  она должна вывести справку о своём использовании и завершить работу.
...
- Если программа запущена с параметрами командной строки,
  она должна вывести в стандартный поток вывода «Invalid argument» и завершиться с кодом возврата 1.
- Если программа запущена без параметров (режим stdin), она должна вывести в стандартный поток вывода «Invalid argument»
  и завершиться с кодом возврата 0. Это необходимо для корректного тестирования программы в системе Яндекс.Contest.
...
где **byte** – число в десятичной системе, представляющее байт (в диапазоне от 0 до 255),
**number of bits** — количество битов для циклического вращения,
а **L** или **R** указывает направление вращения (влево или вправо).

Считывает байт и количество битов из стандартного ввода:
...
Объяснение: число 17 в двоичной системе равно 00010001. При циклическом сдвиге на 2 бита влево получается 01000100, что соответствует числу 68.
...
Объяснение: число 255 в двоичной системе — это 11111111. При циклическом сдвиге на 1 бит вправо получается 01111111,
что соответствует числу 127.
...
Объяснение: число 300 выходит за пределы допустимого диапазона значений байта от 0 до 255.
